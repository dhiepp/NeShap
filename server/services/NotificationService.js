...
module.exports = class NotificationService {
	static async from_post(user_id, post_id, type, amount) {
		try {
			...
			case 'like': {
				...
				if (amount > 1) {
					content = `Và ${amount - 1} người khác đã thích bài viết của bạn.`;
				}
				break;
			}
			case 'comment': {
				content = `Đã bình luận về bài viết của bạn (${amount} bình luận).`;
				break;
			}
			}
	...
	static async from_friend(user_id, friend_id, f1) {
		try {
			let content;
			if (f1) content = 'Đã chấp nhận lời mời kết bạn.';
			else content = 'Đã gửi cho bạn lời mời kết bạn.';
